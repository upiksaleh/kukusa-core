...
class m200220_001200_rbac extends \Kukusa\Db\Migration
{
    /** @var \yii\rbac\BaseManager */
    private $authManager;
    protected $useColumnInfo = false;
    public function up(){
//        Kukusa::$app->authManager = ['class'=>'yihai\core\rbac\DbManager'];
        $this->authManager = Kukusa::createObject(['class'=>DbManager::class]);
        $this->_db_up();
...
//        $this->authManager = Kukusa::$app->getAuthManager();
        $this->authManager = Kukusa::createObject(['class'=>DbManager::class]);
//        if ($this->authManager instanceof DbManager) {
        $this->_db_down();
//        }elseif($this->authManager instanceof PhpManager){

        $this->authManager = Kukusa::createObject(['class'=>PhpManager::class]);
...
//        }
    ...
    public function _db_up()
    {
        ...
        if(!$authManager->getRole(RbacHelper::roleSuperuserName())){
            $role = $authManager->createRole(RbacHelper::roleSuperuserName());
            $role->description = 'Role Superuser';
