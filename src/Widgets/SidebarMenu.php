...
class SidebarMenu extends Widget
{
    ...
    public function run()
    {
        if ($this->route === null && Kukusa::$app->controller !== null) {
            $this->route = Kukusa::$app->controller->getRoute();
        }
        if ($this->params === null) {
            $this->params = Kukusa::$app->request->getQueryParams();
        }
        $posDefaultAction = strpos($this->route, Kukusa::$app->controller->defaultAction);
        if ($posDefaultAction) {
            $this->noDefaultAction = rtrim(substr($this->route, 0, $posDefaultAction), '/');
        } else {
            $this->noDefaultAction = false;
        }
        $posDefaultRoute = strpos($this->route, Kukusa::$app->controller->module->defaultRoute);
        if ($posDefaultRoute) {
    ...
    protected function renderItems($items)
    {
        ...
        foreach ($items as $i => $item) {
            ...
            ArrayHelper::multisort($item->children, ['pos']);
            ...
            $replacements = [
                ...
                '{url}' => $item->getRouteBuild() ? Url::to($item->getRouteBuild()) : 'javascript:void(0);',
                '{items}' => (!empty($item->children) ? $this->renderItems($item->children) : ''),
    ...
    {
//        print_r($this->items->children);exit;
        foreach ($items->children as $i => $item) {
            if (!empty($item->permissions)) {
                $can = false;
                foreach ($item->permissions as $permission) {
                    if (Kukusa::$app->user->can($permission)) {
                        $can = true;
                        continue;
                    }
                }
                if (!$can) {
                    unset($items->children[$i]);
                    continue;
            ...
            $encodeLabel = $item->encodeLabel ? $item->encodeLabel : $this->encodeLabels;
            $items->children[$i]->label = $encodeLabel ? Html::encode($item->label) : $item->label;
            $items->children[$i]->icon = $item->icon ? $item->icon : '';
            $hasActiveChild = false;
            if (!empty($item->children)) {

                $items->children[$i]->children = $this->normalizeItems($item, $hasActiveChild);
                if (empty($items->children[$i]->children) && $this->hideEmptyItems) {
                    unset($items->children[$i]->children);
                    if (!($item->route)) {
                        unset($items->children[$i]);
                        continue;
                    }
            ...
            if (!$item->isActive) {
                if ($this->activateParents && $hasActiveChild || $this->activateItems && $this->isItemActive($item)) {
                    $active = $items->children[$i]->isActive = true;
                } else {
                    $items->children[$i]->isActive = false;
                }
            } elseif ($item->isActive) {
                $active = true;
            }
        }
        return array_values($items->children);
    }
    ...
    protected function isItemActive($item)
    {
        if ($item->route && is_array($item->route) && isset($item->route[0])) {
            $route = $item->route[0];
            $route = ltrim($route, '/');
            if ($item->isModelRepo && Kukusa::$app->modelRepo->currentModel) {
                if ($item->activeRoute === Kukusa::$app->modelRepo->currentModel::modelRepoUrlPath())
                    return true;
                else return false;
            }

            $unique_route = Kukusa::$app->controller->getRoute();

            if (!$item->isModelRepo && $item->activeRoute) {
            ...
            if (Kukusa::$app->controller->getUniqueId() == $route)
                return true;
