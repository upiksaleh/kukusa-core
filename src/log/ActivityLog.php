...
use Kukusa\Helpers\Json;
use Kukusa\Web\Application;
...
class ActivityLog extends ActiveRecord
{
    ...
    public static function newLog($type, $owner = null, $msg = null, $save = true)
    {
        ...
        $log = new static();
        $log->type = $type;
        if (Kukusa::$app instanceof Application) {
            if (Kukusa::$app->user->isGuest)
                $log->user = '0|guest|guest';
            else {
                $user = Kukusa::$app->user->identity->model->id . '|' . Kukusa::$app->user->identity->model->username;
                $log->user = $user;
            }
            $log->ip = Kukusa::$app->request->getRemoteIP();
        } else {
            $log->user = '-1|console|console';
            $log->ip = '127.0.0.1';
        }
        if (Kukusa::$app->controller && Kukusa::$app->controller->action)
            $log->action = Kukusa::$app->controller->action->getUniqueId();
        else {
...
//            'baseTitle' => Kukusa::t('yihai', 'Log aktivitas'),
//            'actionCreate' => false,
//            'actionUpdate' => false,
//            'actionDelete' => false,
